...
function FindingRow({ f, agentLabel }: { f: Finding; agentLabel: string }) {
  return (
    <Link href={`/finding/${f.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
      <div style={{
        display: 'grid',
              ...
              {visibleFindings.map((f, i) => {
                ...
                return (
                  <div key={f.id} style={{ borderTop: i === 0 ? 'none' : undefined }}>
                    <FindingRow f={f} agentLabel={agentLabel} />
                  </div>
                );
              })}
